...
function App() {
  ...
  const filteredUnits = useMemo(() => {
    return [...new Set(allData.filter(d => d.unitGroup?.startsWith(selectedGrade)).map(d => d.unitGroup))];
  ...
  const submitQuizAnswer = () => {
    ...
    const record = { q: (mode === 'ja-en' ? item.ja : item.en), a: currentInput, correct: correct, ok: isCorrect };
  ...
  const sendQuizResultToGAS = async (finalAnswers) => {
    ...
    try { await axios.post(LOG_GAS_URL, JSON.stringify(resultData), { headers: { 'Content-Type': 'text/plain' } }); } 
